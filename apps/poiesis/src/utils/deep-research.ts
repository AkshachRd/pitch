...
export function generateSerpQueriesPrompt(query: string) {
    ...
    const outputSchema = JSON.stringify(zodToJsonSchema(SERPQuerySchema), null, 4);
...
export function processSearchResultPrompt(query: string, researchGoal: string, results: Source[]) {
...
export function reviewSerpQueriesPrompt(query: string, learnings: string[], suggestion: string) {
    const SERPQuerySchema = getSERPQuerySchema();
    const outputSchema = JSON.stringify(zodToJsonSchema(SERPQuerySchema), null, 4);
...
export function writeFinalReportPrompt(query: string, learnings: string[], requirement: string) {
    const learningsString = learnings
        .map((learning) => `<learning>\n${learning}\n</learning>`)
        .join('\n');
    return [
