...
function useDeepResearch() {
    ...
    async function runSearchTask(queries: SearchTask[]) {
        ...
        const getProviderOptions = () => {
            ...
            if (enableSearch && searchProvider === 'model' && provider === 'openrouter') {
                return {
                    openrouter: {
                        plugins: [
                            {
    ...
    async function writeFinalReport() {
        const { language } = useSettingStore.getState();
        const { query, tasks, setId, setTitle, setSources, requirement } = useTaskStore.getState();
        ...
        setTitle(title);
        const sources = flat(tasks.map((item) => (item.sources ? item.sources : [])));
        setSources(sources);
