...
type BochaSearchOptions = {
    freshness?: 'oneDay' | 'oneWeek' | 'oneMonth' | 'oneYear' | 'noLimit' | string;
    summary?: boolean;
    count?: number;
    page?: number;
};
...
function useWebSearch() {
    async function tavily(query: string, options: TavilySearchOptions = {}) {
        const { mode, tavilyApiKey, tavilyApiProxy, searchMaxResult, accessPassword } =
    ...
    async function firecrawl(query: string, options: FirecrawlSearchOptions = {}) {
        const {
            mode,
            firecrawlApiKey,
            firecrawlApiProxy,
        ...
        const response = await fetch(
            mode === 'local'
                ? `${completePath(firecrawlApiProxy || FIRECRAWL_BASE_URL, '/v1')}/search`
                : '/api/search/firecrawl/v1/search',
            {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    Authorization: `Bearer ${mode === 'local' ? firecrawlKey : accessKey}`,
                },
                credentials: 'omit',
                body: JSON.stringify({
                    query,
    ...
    async function bocha(query: string, options: BochaSearchOptions = {}) {
        const { mode, bochaApiKey, bochaApiProxy, searchMaxResult, accessPassword } =
            useSettingStore.getState();
        const accessKey = generateSignature(accessPassword, Date.now());
    ...
    async function searxng(query: string, options: SearxngSearchOptions = {}) {
        ...
        return (results as SearxngSearchResult[])
            .filter(
                (item, idx) =>
                    item.content && item.url && idx < searchMaxResult * 5 && item.score >= 0.5,
            )
            .map((result) => pick(result, ['title', 'content', 'url'])) as Source[];
    }
