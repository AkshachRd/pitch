'use client';
import { useRef, useLayoutEffect, useState, type ReactNode, useEffect } from 'react';
...
function MilkdownEditor(props: EditorProps) {
    ...
    return (
        <div className={cn('relative', className)} ref={containerRef}>
            <div
                className={cn('milkdown-editor prose prose-slate dark:prose-invert max-w-full', {
                    hidden: mode !== 'WYSIWYM',
                })}
                ref={milkdownEditorRef}
            ></div>
            <div
                className={cn('markdown-editor whitespace-break-spaces text-base print:hidden', {
                    hidden: mode !== 'markdown',
                })}
                ref={markdownEditorRef}
            ></div>
            <FloatingMenu targetRef={containerRef} fixedTopOffset={16}>
                <div className="flex flex-col gap-1 rounded-full border bg-white p-1 py-2 opacity-50 hover:opacity-100 max-sm:opacity-80 dark:bg-slate-800 print:hidden">
                    {mode === 'WYSIWYM' ? (
                        <Button
