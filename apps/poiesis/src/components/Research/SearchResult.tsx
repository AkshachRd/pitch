...
import { LoaderCircle, CircleCheck, TextSearch, Download, Trash } from 'lucide-react';
...
function ResearchGoal({ milkdownEditor, goal }: { milkdownEditor?: Crepe; goal: string }) {
...
function SearchResult() {
    ...
    return (
        <section className="mt-4 rounded-md border p-4 print:hidden">
            <h3 className="mb-2 border-b text-lg font-semibold leading-10">
                        ...
                        {taskStore.tasks.map((item, idx) => {
                            return (
                                <AccordionItem key={idx} value={item.query}>
                                    ...
                                    <AccordionContent className="prose prose-slate dark:prose-invert min-h-20 max-w-full">
                                        <ResearchGoal
